...
class Authentication(object):
    ...
    __ROLE_GROUP = {
        'normal': {
            'izi_hr_security.group_hr_employee_director': 'hr_employee_director',
            'izi_hr_security.group_hr_employee_deputy_director': 'hr_employee_deputy_director',
            'izi_hr_security.group_hr_employee_prefect': 'hr_employee_prefect',
            'izi_hr_security.group_hr_employee_normal': 'hr_employee_normal',
        },
        'hr': {
            'izi_hr_security.group_hr_manager_prefect': 'hr_manager_prefect',
            'izi_hr_security.group_hr_manager_department': 'hr_manager_department',
            'izi_hr_security.group_hr_manager_officer': 'hr_manager_officer',
            'izi_hr_security.group_hr_manager_legislation': 'hr_manager_legislation',
            'izi_hr_security.group_hr_manager_administrative': 'hr_manager_administrative',
        },
    }
    ...
    def verify_access_token(env, access_token):
        try:
            ...
            raise ApiException(_('Access token không hợp lệ'), ApiException.INVALID_ACCESS_TOKEN)
    ...
    def user_can_login_multi_device(user, device_id):
        ...
        if user.access_device_number == 0:
            raise ApiException(_('Tài khoản hiện tại không được phép đăng nhập trên thiết bị di động'),
                               ApiException.NOT_ALLOWED_DEVICE)

        current_device_number = user.device_ids.search_count(
            [('device_id', '!=', device_id), ('user_id', '=', user.id)])
        if current_device_number >= user.access_device_number:
            raise ApiException(_('Tài khoản đăng nhập hiện tại vượt quá số lượng thiết bị di động được phép'),
    ...
    def is_director(user):
        if user.has_group('izi_base.group_supervisor'):
            return True
        return False

    @staticmethod
    def is_branch_user(user):
        if user.has_group('izi_branch.group_branch_user'):
            return True
        return False

    @staticmethod
    def is_branch_manager(user):
        if user.has_group('izi_branch.group_branch_user_manager'):
            return True
        return False
